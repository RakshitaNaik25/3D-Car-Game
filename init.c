...
void gbps_InitWindow(int argc,char** argv)
{
    ...
    //gbps_InitFog();
    gbps_LoadResources();
    gbps_InitAudio(); // Initialize audio system
    srand(time(0));  // Fixed: Initialize random seed once globally
    ...
    {
        file = fopen("score.txt","wb");  // Fixed: = instead of ==
        if(file != NULL)
        {
            long zero = 0;
            fwrite(&zero,sizeof(long),1,file);  // Fixed: write address of zero, not literal 0
            fclose(file);
        }
    }
}
...
void gbps_LoadResources()
{
    ...
    if (!tyre1)
    {
        ...
        //if (!pmodel1) exit(0);
        //glmUnitize(pmodel1);// This will rescale the object to fit into the unity matrix
        //printf("Loading tyre... done\n");
    }
}

void gbps_CleanupResources()
{
    // Cleanup textures
    glDeleteTextures(20, GBPS_texArr);
    
    // Cleanup models
    if (car1) glmDelete(car1);
    if (tyre1) glmDelete(tyre1);
}
